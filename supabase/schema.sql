...
begin
  ...
  -- Todo lo relativo a la visita en sí (quién la atiende, cuándo, y qué se le
  -- va a hacer) se puede ajustar mientras no la hayan atendido: la clienta
  -- cambia de opinión, o llama días después a agregar un servicio. Una vez
  -- completada o cancelada, queda congelado como registro histórico.
...
-- El personal debe poder ver sus propios abonos para que "Mi perfil"
-- calcule el saldo pendiente igual que la pantalla de Préstamos del admin.
...
-- que las ventas y los préstamos de insumo).
...
-- ---------------------------------------------------------
-- 16. Abonos adicionales a una cita: la clienta abona al agendar y vuelve
--     días después a pagar otra parte antes del servicio. No se suma dentro
--     de citas.abono porque ese campo tiene UNA fecha y UN medio de pago:
--     cada abono adicional guarda los suyos y se cuadra el día que entró.
-- ---------------------------------------------------------
create table public.cita_abonos (
  id uuid primary key default gen_random_uuid(),
...
create index idx_cita_abonos_cita on public.cita_abonos(cita_id);
create index idx_cita_abonos_created on public.cita_abonos(created_at);
